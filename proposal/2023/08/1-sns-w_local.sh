ic-admin  \
   --nns-url "http://localhost:$(dfx info replica-port)" \
   --secret-key-pem ~/.config/dfx/identity/"$(dfx identity whoami)"/identity.pem \
   propose-to-update-sns-deploy-whitelist  \
   --added-principals "bnz7o-iuaaa-aaaaa-qaaaa-cai"  \
   --proposer "298679396557238448" \
   --proposal-title "Enable principal bnz7o-iuaaa-aaaaa-qaaaa-cai in SNS-W to initiate the generation of an SNS for EMC"  \
   --summary "## Summary
